...
function TodosPage() {
  ...
  return (
    <main className="relative flex h-svh w-full flex-col bg-background">
      {/* Header */}
      <header
      ...
      <Sheet open={navOpen} onOpenChange={setNavOpen}>
        <SheetContent side="left" className="w-[82%] max-w-sm p-0">
      ...
      {/* Search */}
      <div className="relative bg-card px-4 pb-3">
      ...
      {/* Tag filter chips */}
      {allTags.length > 0 && (
        <div className="flex gap-1.5 overflow-x-auto bg-card px-4 pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <button
      ...
      {view.kind === "myday" && activeRaw.length > 0 && (
        <div className="flex items-center gap-2 bg-card px-4 pb-2">
          <button
      ...
      {/* Composer */}
      <div className="border-t border-border bg-card px-4 py-3 pr-[calc(1rem+4.5rem)] sm:pr-4">
        <div
          role="status"
          aria-live="polite"
          aria-atomic="true"
